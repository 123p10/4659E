...
void drivePID(int err){
	...
	while(abs(SensorValue[leftEncoder]) < abs(err) - tolerance || abs(SensorValue[rightEncoder]) < abs(err) - tolerance){
		...
		if(abs(err) - abs((SensorValue[leftEncoder] * 35.3)/10) < 30){
			leftMotor -= leftMotor * 0.2;
			rightMotor -= rightMotor * 0.2;
		}
...
task clawControl(){
	//Opened and closed positions
	 ...
	 int LEFT_OPENED  = 2000;  //Pot value @ left  claw opened position
	 int RIGHT_CLOSED = 300; //Pot value @ right claw closed position 240s
	 int RIGHT_OPENED = 2000;//Pot value @ right claw opened position
	...
	while(true){
		...
		if(vexRT[Btn6U] == 1 || clawState == CLAW_CLOSE){ //Command close
			...
			if(SensorValue[leftClawPot] < LEFT_OPENED){
				leftSignal = 127;
				if(abs(SensorValue[leftClawPot] - LEFT_OPENED) < 500){
					leftSignal -= leftSignal / 4;
				}
			}else{
				leftSignal = 0;
			}
			if(SensorValue[rightClawPot] < RIGHT_OPENED){
				rightSignal = 127;
					if(abs(SensorValue[rightClawPot] - RIGHT_OPENED) < 500){
						rightSignal -= rightSignal / 4;
				}
			}else{
				rightSignal = 0;
 			}
...
task liftControl(){
	...
	const int LIFT_UP   = 3800; //Pot value @ raised position
	const int LIFT_DOWN = 500; //Pot value @ lowest position
...
//	bool macro2 = false;
  ...
  while (true)
  {
	...
	//	clearLCDLine(2);
		...
		if(button == 1){
			displayLCDString(0,0,"Team: 4659E")
		//	displayLCDChar(1,0,nCurrCortexProgramType);
		//	displayNextLCDChar(nCurrCortexProgramType);
		...
		if(button == 2){
			...
			displayLCDString(1,0,powerExpander);
		//	displayNextLCDString("V")
		}
		if(button == 4){
		//	displayLCDString(0,0,"Autonomous Type: ");
			if(auton == AUTON_COMP){
				displayLCDString(0,0,"Comp Auton");
			}
			else if(auton == AUTON_SKILLS){
				displayLCDString(0,0,"Auton Skills")
			}
		//	displayLCDChar(0,0,auton);
		...
		if(vexRT[Btn7U]){
			...
			clawError[9] = (abs(SensorValue[leftClawPot] + SensorValue[rightClawPot]))/2;
			...
			if(SensorValue[liftPot] < 1600 && clawError){
				lift = 40;
			}
			if(1200 - SensorValue[liftPot] < 10){
				if(SensorValue[leftClawPot] < 2000){
	        lClaw = 127;
				}
				else{
					lClaw = 0;
				}
				if(SensorValue[rightClawPot] < 2000 ){
					rClaw = 127;
				}
				else{
					rClaw = 0;
		...
		motor[leftClaw] = lClaw;
		motor[rightClaw] = rClaw;
